...
import { Scale, LogOut, User, Bell, Settings, Building, Menu, X, UserCheck, Home, Users, BarChart3 } from 'lucide-react';
...
const Header: React.FC<HeaderProps> = ({ currentUser, onLogout, currentView, onViewChange }) => {
  ...
  const getNavigationItems = () => {
    ...
    // Dashboard is available to all roles except clients
    ...
    // Firm analytics for firm admins and lawyers
    if (currentUser.role === 'firm-admin' || currentUser.role === 'lawyer') {
      items.push({
        id: 'firm-dashboard',
        label: 'Firm Analytics',
        icon: BarChart3,
        description: 'Firm performance and analytics'
      });
    }
    
    // Firm management only for firm admins
  ...
  const shouldShowNavigation = navigationItems.length > 1;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-3 sm:py-4">
          ...
          {shouldShowNavigation && (
            <div className="hidden md:flex items-center space-x-1 flex-1 justify-center max-w-md mx-8">
